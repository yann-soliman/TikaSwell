...
- Open-Meteo remains the weather/marine provider for wind, waves, swell, and wind-wave conditions.
- api-maree.fr is used only for tide water levels and derived tide context.
- Tide data remains cache-first in SQLite because heights for a given date are stable.
- The api-maree.fr key must be injected through `API_MAREE_API_KEY` and must not be logged or committed.
